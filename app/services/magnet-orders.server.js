    ...
    for (const orderEdge of orders) {
      ...
      try {
        ...
        if (!shop || !accessToken) {
          ...
          console.warn(
            `⚠️ Shop: ${shop}, AccessToken: ${
              accessToken ? "EXISTS" : "MISSING"
            }`
          );
        } else {
          console.log(
            `🔍 Fetching REST API details for order ${order.name} (ID: ${orderId}, Shop: ${shop})`
          );
          ...
          console.log(
            `📡 REST API Response status: ${restResponse.status} for order ${order.name}`
          );
          ...
          if (restResponse.ok) {
            ...
            console.log(
              `📋 Full REST API response for order ${order.name}:`,
              JSON.stringify(restOrder, null, 2)
            );

            if (restOrder && restOrder.line_items) {
              console.log(
                `📦 REST API - Order ${order.name} has ${restOrder.line_items.length} line items`
              );

              for (const lineItem of restOrder.line_items) {
                console.log(
                  `🔍 Checking line item: ${
                    lineItem.title || lineItem.name || "Unknown"
                  }`
                );
                console.log(
                  `🔍 Line item full data:`,
                ...
                if (lineItem.properties && lineItem.properties.length > 0) {
                  ...
                  console.log(
                    `🔑 Properties for order ${order.name}, line item:`,
                  ...
                  if (magnetKeyProperty && magnetKeyProperty.value) {
                    ...
                    console.log(
                      `✅ Found magnet key: ${magnetKey} for order ${order.name}`
                    );
                    break;
                  } else {
                    console.log(
                      `⚠️ magnet_upload_key not found in properties for order ${order.name}`
                    );
                    console.log(
                      `⚠️ Available property names:`,
                      propertiesArray.map((p) => p.name).join(", ")
                    );
                  }
                } else {
                  console.log(
                    `⚠️ Line item has no properties (empty array or null) for order ${order.name}`
                  );

                  if (restOrder.note_attributes && restOrder.note_attributes.length > 0) {
                    console.log(
                      `🔍 Checking note_attributes for order ${order.name}:`,
                      JSON.stringify(restOrder.note_attributes, null, 2)
                    );
                    ...
                    if (magnetKeyAttr && magnetKeyAttr.value) {
                      ...
                      console.log(
                        `✅ Found magnet key in note_attributes: ${magnetKey} for order ${order.name}`
                      );
                    }
                  }
              ...
              console.warn(
                `⚠️ REST API - Order ${order.name} has no line_items`
              );
            }
          } else {
            const errorText = await restResponse.text();
            console.warn(
              `⚠️ REST API returned ${restResponse.status} for order ${order.name}:`,
              errorText
            );
          }
        }
        ...
        console.error(
          `❌ REST API error for order ${order.name}:`,
          restError.message
        );
        console.error(`❌ REST API error stack:`, restError.stack);
      }

      const shouldInclude = magnetKey || true; // şimdilik hepsini göster
      ...
      if (shouldInclude) {
        ...
        if (magnetKey) {
          try {
            ...
            console.error(
              `Cloudflare'den fotoğraf sayısı alınırken hata (key: ${magnetKey}):`,
              err
            );
          }
        }
